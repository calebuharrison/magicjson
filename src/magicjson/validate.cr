  ...
  macro validate
    \{% begin %}
      ...
      \{% for k, v in MAGICJSON_FIELDS %}
        \{% if ((MAGICJSON_DEFAULTS[:dont_deserialize] && v[:dont_deserialize] == nil) || v[:dont_deserialize] == true) && v[:default] == nil %}
        ...
        \{% if ((MAGICJSON_DEFAULTS[:emit_null] && v[:emit_null] == nil) || v[:emit_null] == true) &&
          ...
          (!v[:type].is_a?(Union) ||
            v[:type].types.any? do |t|
              ...
              t.is_a?(Path) && t.names.size == 1 && t.names[0] == "Nil".id && t.resolve == ::Nil
            end) %}
